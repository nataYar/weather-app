...
function App() {
  ...
  useEffect(() => {
    if (lat && long) {
      fetch(`${baseUrl}/locations/v1/cities/geoposition/search?apikey=${apiKey}&q=${lat}%2C${long}`)
        .then((res) => res.json())
        .then((res) => {
    ...
    if (locationKey) {
      ...
      imagesApi
        ...
        .then(({ data }) => {
          ...
          if (data.length > 0) {
            data[0].image_type === "vector" ? setCityURL(data[1].assets.huge_thumb.url) : setCityURL(data[0].assets.huge_thumb.url);
          } else if (data.length === 0) {
            console.log("no pictures");

            imagesApi.searchImages(queryParamsForCountry).then(({ data }) => {
              JSON.stringify(data, null, 5);
  ...
  return (
    <div className="app-background">
      <div className="dashboard">
        <div className="upper-panel" style={{ backgroundImage: "url(" + `${cityURL}` + ")", backgroundSize: "cover", backgroundPosition: "center" }}>
          <div className="upper-right-section">
            ...
            <div className="temp-type-section">
              <div className={tempType === "F" ? "temp-type temp-type-on" : " temp-type temp-type-off"} onClick={() => setTempType("F")}>
                F
              </div>
              /
              <div className={tempType === "C" ? "temp-type temp-type-on" : " temp-type temp-type-off"} onClick={() => setTempType("C")}>
                C
              </div>
            </div>
          </div>
        ...
        <div className="forecast-container">
          {weatherData.length > 0 &&
            weatherData.map((day, ind) => (
              <div key={ind}>
                <Forecast min={day.min} max={day.max} date={formatDate(day.date)} icon={day.icon} tempType={tempType} />
              </div>
            ))}
          <LocationSearch
