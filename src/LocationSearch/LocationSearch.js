...
const LocationSearch = ({ onFound }) => {
  ...
  const getLocation = (search) => {
    const url = `${baseUrl}/locations/v1/cities/search?apikey=${apiKey}&q=${search}`;
    fetch(url)
  ...
  return (
    <div className="search-container">
      <input placeholder="Search the city" value={search} onChange={(e) => setSearch(e.target.value)} onKeyUp={(e) => triggerCitySearch(e)} />
        ...
        <div className="selection">
          {cityArray.sort(sortCities).map((city, ind) => (
            <div key={ind} className="city-dropdown" onClick={() => handleSelect(city)}>
              <div>
                {" "}
                {city.country}, {city.region}, {city.name}{" "}
              </div>
            </div>
          ))}
        </div>
