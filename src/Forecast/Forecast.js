...
const Forecast = ({ min, max, icon, date, tempType }) => {
  ...
  useEffect(() => {
    ...
    tl.fromTo(minTempRef.current, { opacity: "0", y: "50%", duration: 1 }, { opacity: "1", y: "0%", duration: 1, ease: "back.out(1.7)" })
    ...
    <div className="day-container">
      ...
      {/* <div className="temperature-container" > */}
      {
        <div className="temperature" ref={tempRef}>
          <div className="minTemp" ref={minTempRef}>
            {tempType == "C" ? minC : min} min{" "}
          </div>
          <div className="sep" ref={sepRef}>
            |
          </div>
          <div className="maxTemp" ref={maxTempRef}>
            {" "}
            {tempType == "C" ? maxC : max} max{" "}
          </div>
        </div>
      }
      {/* </div> */}
